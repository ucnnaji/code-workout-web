...
export const STAGES = ['information', 'consent', 'language', 'pre_survey', 'demo', 'coding', 'post_survey', 'crossover', 'incentive', 'completion'];
export const STAGE_LABELS = { information: 'Research information', consent: 'Consent', pre_survey: 'Pre-survey', demo: 'Practice environment demo', language: 'Language', coding: 'Initial practice', post_survey: 'Post-survey', crossover: 'Crossover practice', incentive: 'Incentive', completion: 'Completion' };
...
export function scoreSelfEfficacy(def, answers) {
    const items = (def.items || []).filter(i => typeof i.id === 'string' && i.id.startsWith('pse_'));
    const scored = [];
    for (const item of items) {
        ...
        const index = item.options.indexOf(response);
        if (index >= 0) scored.push({ id: item.id, value: index + 1 });
    }
    const values = scored.map(x => x.value);
    const coreValues = scored.filter(x => x.id.startsWith('pse_core_')).map(x => x.value);
    const topicValues = scored.filter(x => !x.id.startsWith('pse_core_')).map(x => x.value);
    const avg = xs => xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : null;
    return {
        instrument: def.version,
        scaleMin: 1,
        scaleMax: 7,
        nExpected: items.length,
        nAnswered: values.length,
        nSkipped: items.length - values.length,
        complete: items.length > 0 && values.length === items.length,
        mean: avg(values),
        coreMean: avg(coreValues),
        coreAnswered: coreValues.length,
        coreExpected: items.filter(i => i.id.startsWith('pse_core_')).length,
        topicMean: avg(topicValues),
        topicAnswered: topicValues.length,
        topicExpected: items.filter(i => !i.id.startsWith('pse_core_')).length,
        possibleRange: items.length ? '1-7 mean' : null,
        note: 'Adapted IPSES-derived programming self-efficacy measure. The six-item core mean is the common repeated indicator; the two-item topic mean is descriptive. Missing responses are not imputed.'
    };
}
