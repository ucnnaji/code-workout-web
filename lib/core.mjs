...
export function scoreSelfEfficacy(def, answers) { const items = (def.items || []).filter(i => i.id.startsWith('gse_')), values = items.filter(i => answers[i.id] != null).map(i => i.options.indexOf(answers[i.id]) + 1), sum = values.reduce((a, b) => a + b, 0); return { instrument: def.version, nExpected: items.length, nAnswered: values.length, nSkipped: items.length - values.length, total: items.length > 0 && values.length === items.length ? sum : null, mean: values.length ? sum / values.length : null, possibleRange: items.length ? `${items.length}-${items.length * 4}` : null, note: 'Validated GSE total is reported only for complete responses; partial-response mean is descriptive and is not an imputed total.' }; }
