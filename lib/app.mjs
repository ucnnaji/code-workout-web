...
const DEFAULT_PROBLEM_SOLVING_INPUT = Object.freeze({
    id: 'program_input',
    ...
    default: '',
    required: false,
    help: 'Use this only if your program reads input. You may enter text or a number.'
});

function normalizeQuestionSnapshot(question = {}, modality = null) {
    const q = { ...(question || {}) };

    // Code Explanation should ask students to explain the code,
    // not simply predict its output.
    if (modality === 'code-explanation')
        q.prompt = CODE_EXPLANATION_PROMPT;

    // Problem Solving receives a generic optional stdin field
    // unless that question already defines its own structured inputs.
    if (
        modality === 'problem-solving' &&
        (!Array.isArray(q.inputSchema) || q.inputSchema.length === 0)
    ) {
        q.inputSchema = [
            { ...DEFAULT_PROBLEM_SOLVING_INPUT }
        ];
    }
...
export function createApp({ db, env = process.env, providers = createProviders(env), startupIssues = [] }) {
    ...
    async function codingContext(req) {
    const sid = uuid(req.body.sessionId);
    const d = await rawState(req, sid);

    requireCurrent(d, 'coding');

    const a = await db.call('assignment_get', {
        ...params(req, sid),
        assignmentId: uuid(req.body.assignmentId)
    });
    ...
    return {
        ...
        a,
        sid,
        question: normalizeQuestionSnapshot(
            a.question_snapshot,
            a.modality_id
        )
    };
    ...
    app.post('/api/modality/start', async (req, res, next) => {
        try {
            ...
            const questions = ids.map(id => normalizeQuestionSnapshot({ ...bank.find(x => x.id === id), inputSchema: bank.find(x => x.id === id).inputSchema || inputSchemas.questions[id] || [], bankVersion: pinnedBankHash || s.protocol_snapshot.config.questionBankVersion || 'legacy-current-bank', questionVersion: hashObject(bank.find(x => x.id === id)).slice(0, 16) }, modality));
