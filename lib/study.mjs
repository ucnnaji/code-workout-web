...
function surveyDefinition(phase, language, number, cfg) {
    ...
    if (pre && Array.isArray(surveys.background) && (!cfg.backgroundFirstSessionOnly || number === 1))
        items.push(...surveys.background);

    if (Array.isArray(surveys.selfEfficacyCore))
        items.push(...surveys.selfEfficacyCore);

    items.push(...(surveys.selfEfficacyBySession?.[String(number)] || []));

    if (cfg.includeAdditionalSurveyItems) {
        const extras = pre ? surveys.preExtras : surveys.postExtras;
        if (Array.isArray(extras)) items.push(...extras);
    }

    // Apply the selected programming language consistently to both surveys.
    // The PSE items are intentionally language-neutral; this preserves legacy
    // Python-to-Java wording adaptation for background/optional items if needed.
    items = structuredClone(items).map(i => {
        let label = i.label;
        let source = i.source;
        if (language === 'java' && typeof label === 'string') {
            const adapted = label.includes('Python') || label.includes('CS 1400');
            label = label.replaceAll('Python', 'Java').replaceAll('CS 1400', 'CS 1410');
            if (adapted) source = `${source || ''}; Java wording adaptation needs review`;
        }
        return { ...i, label, source };
    });

    return {
        version: `${surveys.version}-session-${number}-${language}-${phase}`,
        items,
        prompt: pre ? surveys.prePrompt : surveys.postPrompt,
        scaleNote: surveys.scaleNote
    };
}
