...
function surveyDefinition(phase, language, number, cfg) {
    ...
    // Apply the selected programming language consistently
    // to BOTH pre-survey and post-survey items.
    items = structuredClone(items).map(i => {
        ...
        return {
            ...
            source:
                language === 'java' &&
                (i.label.includes('Python') || i.label.includes('CS 1400'))
                    ? `${i.source}; Java wording adaptation needs review`
                    : i.source
        };
    });

    return {
        version: surveys.version + '-' + language + '-' + phase,
        items,
