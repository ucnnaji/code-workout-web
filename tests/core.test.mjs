...
test('active question seed contains no participant-provided runtime stdin tasks', () => { const bank = JSON.parse(readFileSync(new URL('../questions.seed.json', import.meta.url), 'utf8')); for (const q of bank.filter(q => ['problem-solving','debugging','code-explanation'].includes(q.modality_id))) { const text = `${q.title}\n${q.prompt}\n${q.starter_code}`; assert.doesNotMatch(text, /\binput\s*\(|Scanner\s+\w+\s*=\s*new\s+Scanner\s*\(\s*System\.in|\.next(?:Int|Double|Line)?\s*\(/, `stdin-dependent question remains: ${q.id}`); } });
